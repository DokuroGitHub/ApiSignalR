...
public class MessageEmoteBeforeDeleteEventHandler : INotificationHandler<MessageEmoteBeforeDeleteEvent>
{
    private readonly ILogger<MessageEmoteBeforeDeleteEventHandler> _logger;
    ...
    public MessageEmoteBeforeDeleteEventHandler(
        ILogger<MessageEmoteBeforeDeleteEventHandler> logger,
    ...
    public async Task Handle(MessageEmoteBeforeDeleteEvent notification, CancellationToken cancellationToken)
    {
        ...
        var allMessageEmotesHubTask = Task.Run(async () =>
        {
            var items = await _unitOfWork.MessageEmoteRepository.GetAllAsync<MessageEmoteBriefDto>(
                where: x =>
                    x.MessageId != notification.Item.MessageId &&
                    x.UserId != notification.Item.UserId,
